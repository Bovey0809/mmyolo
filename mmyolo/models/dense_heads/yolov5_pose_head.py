...
class YOLOv5PoseHeadModule(BaseModule):
...
@MODELS.register_module()
class YOLOv5PoseHead(BaseDenseHead):
    ...
    def predict_by_feat(self,
                        ...
                        kpts_preds: List[Tensor] = None,
                        vis_preds: List[Tensor] = None,
        ...
        Args:
            ...
            objectnesses (list[Tensor], Optional): Score factor for
                ...
                (batch_size, 1, H, W).
            kp_preds (list[Tensor]): Keypoint predictions for all
                scale levels, each is a 4D-tensor, has shape
                (batch_size, num_priors * 2, H, W).
            vis_preds (list[Tensor]): Visibility predictions for all
                scale levels, each is a 4D-tensor, has shape
        ...
        for (bboxes, scores, objectness,
            ...
            pad_param = img_meta[
                'pad_param'] if 'pad_param' in img_meta else None
    ...
    def loss_by_feat(
        ...
        Args:
            ...
            objectnesses (Sequence[Tensor]): Score factor for
                all scale level, each is a 4D-tensor, has shape
                (batch_size, 1, H, W).
            kpt_preds (Sequence[Tensor]): Keypoint predictions for each scale
                level, each is a 4D-tensor, the channel number is
                num_priors * num_keypoints * 2.
            vis_preds (Sequence[Tensor]): Visibility predictions for each scale
                level, each is a 4D-tensor, the channel number is
                num_priors * num_keypoints.
        ...
        for i in range(self.num_levels):
            ...
            retained_kpt_pred = kpt_preds[i].reshape(batch_size,
                                                     ...
                                                     h, w)[img_inds,
                                                           ...
                                                           grid_y_inds,
                                                           grid_x_inds]
            decoded_kpt_pred = self._decode_kpt(retained_kpt_pred)
            ...
            # vis loss
            retained_vis_pred = vis_preds[i].reshape(batch_size,
                                                     self.num_base_priors, -1,
                                                     h, w)[img_inds,
                                                           priors_inds, :,
                                                           grid_y_inds,
        ...
        return dict(
            ...
            loss_kpt=loss_kpt * batch_size * world_size,
            loss_vis=loss_vis * batch_size * world_size,
        )
    ...
    def _convert_gt_to_norm_format(self,
        ...
        if isinstance(batch_gt_instances, torch.Tensor):
            batch_targets_normed = (
                self._extracted_from__convert_gt_to_norm_format_6(
                    batch_img_metas, batch_gt_instances))
        else:
            batch_target_list = []
            ...
            for i, gt_instances in enumerate(batch_gt_instances):
                ...
                vis = gt_instances.keypoints_visible

                # normalized to 0-1
                bboxes[:, 1::2] /= img_shape[0]
                bboxes[:, 0::2] /= img_shape[1]
                kpts[..., 1::2] /= img_shape[0]
                kpts[..., 0::2] /= img_shape[1]

                index = bboxes.new_full((len(bboxes), 1), i)
                # (batch_idx, label, normed_bbox)
                target = torch.cat((index, labels[:, None].float(), bboxes),
                                   dim=1)
                # (batch_idx, label, normed_bbox, normed_kpts, vis)
                target = torch.cat((target, kpts.flatten(1), vis.flatten(1)),
                                   dim=1)
                batch_target_list.append(target)

            # (num_base_priors, num_bboxes, 57)
        ...
        # keypoints, vis, prior_ind)
        batch_targets_normed = torch.cat(
            (batch_targets_normed, batch_targets_prior_inds), 2)
        return batch_targets_normed

    # TODO Rename this here and in `_convert_gt_to_norm_format`
    def _extracted_from__convert_gt_to_norm_format_6(self, batch_img_metas,
                                                     batch_gt_instances):
        # fast version
